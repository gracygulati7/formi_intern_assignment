...
# POST endpoint to get detailed info about a specific branch in a city
@router.post("/branch-info")
async def get_branch_info(query: Query):
    """
    Returns detailed branch info for a given city and branch
    """
    try:
        # Sanitize city and branch input
        city = query.city.strip().replace(" ", "")
        branch = query.branch.strip().replace(" ", "")

        # Construct filename: Branch_City.json
